...
export default function App_f() {
            ...
            tx.executeSql('SELECT * FROM local_user;', [],
                (txObj, resultSet) => {
                    ...
                    if (items[0]["light_dark"] == 'light') {
                        ...
                        set_placTextColor('rgba(16, 24, 40, 1)');
                        ...
                        set_work_search_block_BG_cl({backgroundColor: 'rgba(242, 244, 247, 1)'});
                    }
                    else {
                        set_text_main_cl(prevBG => ({ ...prevBG, color: 'rgba(252, 252, 253, 1)' }));
                        set_placTextColor('rgba(252, 252, 253, 1)');
                        ...
                        set_work_search_block_BG_cl({backgroundColor: 'rgba(52, 64, 84, 1)'});
                    }
                });
        });
    };
    ...
    function Request_for_fairy_tales_f() {
        ...
        const fetchDataFromFlaskApp = async (user) => {
            console.log(user);
            try {
                const response = await axios.get(`http://talesofyakutia.ru/api_subscription_fun?api_key=${apiKey}&api_user=${user}`);
                set_data_api(response.data); // Обновляем сразу, без .response.results;
                console.log(response.data);
            } catch (error) {
                console.log('Ошибка при выполнении API-запросаQQQ');
            };
        };
    }
    useEffect(() => {
    ...
    let [dop_ru_sh, set_dop_ru_sh] = useState('');
    ...
    //======================Функция поиска сказки======================//
    let [data_search_api, set_data_search_api] = useState([]);
    let [work_search_block_BG_cl, set_work_search_block_BG_cl] = useState({ backgroundColor: 'rgba(242, 244, 247, 1)' });
    let [search_text_cl, set_earch_text_cl] = useState({
        width: '100%',
        fontSize: 20,
        color: 'rgba(16, 24, 40, 1)',
    });
    let [work_search_open_close_cl, set_work_search_open_close_cl] = useState({display: 'none'});
    function handleTextSubmit_f(text) {
        set_work_search_open_close_cl({display: 'flex'});
        let work_array = [];
        for (let i = 0; i < data_api.length; i++) {
            let work_text = text.split('');
            let work_data
            if (dop_ru_sh == 'ru') {
                work_data = data_api[i][1].split('');
            }
            else {
                work_data = data_api[i][2].split('');
            }
            let on_off_push = true;
            for (let n = 0; n < work_text.length; n++) {
                if (work_text[n].toUpperCase() != work_data[n].toUpperCase()) {
                    on_off_push = false;
                    break;
                }
            };
            if (on_off_push == true) {
                work_array.push([i, data_api[i][1], data_api[i][2]]);
            }
            console.log(work_array);
        }
        set_data_search_api(work_array);
        console.log(work_array);
        console.log(text);
    };

    function handleTextSubmit_close_f() {
        set_work_search_open_close_cl({display: 'none'});
    }

    return (
        <View style={st_catalog.catalog_cl}>
            <View style={[st_catalog.work_search_cl, work_search_open_close_cl]}>
                <FlatList
                    style={st_catalog.gen_flast_cl}
                    data={data_search_api}
                    keyExtractor={(item, index) => index.toString()}
                    renderItem={({ item, index }) => (
                        <View style={[st_catalog.work_search_block_cl, work_search_block_BG_cl]}>
                            <TouchableWithoutFeedback onPress={() => { switching_description_open_f(item[0]), handleTextSubmit_close_f() }}>
                                <View style={work_text_5_cl}>
            ...
            <View style={catalog_mini_1_cl}>
                <View><Text style={text_main_cl}>Библиотека</Text></View>
                <View style={st_catalog.search_block_cl}>
                    <ImageBackground
                        source={require('./search.png')} resizeMode='cover'
                        style={st_catalog.search_img_cl}></ImageBackground>
                    <TextInput
                        placeholderTextColor={placTextColor}
                        placeholder='Поиск'
                        style={search_text_cl}
                        onChangeText={(text) => handleTextSubmit_f(text)}
                        onSubmitEditing={() => handleTextSubmit_close_f()} />
                </View>
                <ImageBackground
                    source={require('./pattern.png')} resizeMode='cover'
                    style={st_catalog.pattern_special_cl}></ImageBackground>
                    ...
                    <FlatList
                        ...
                        <View style={st_catalog.gen_main_block_cl}>
                            <View style={gen_block_cl}>
                                <TouchableWithoutFeedback onPress={() => { switching_description_open_f(index) }}>
                                    <ImageBackground
            ...
            <View style={catalog_mini_2_cl}>
                ...
                <View style={control_work_1_cl}>
                    ...
                    <View style={st_catalog.control_main_button_cl}>
                        <TouchableWithoutFeedback onPress={() => { if (isConnected==true) {listen_to_music_open_f() }}}>
